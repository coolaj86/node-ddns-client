...
DDNS.refreshToken = function (options, cli) {
  return DDNS.login(options, cli).then(function (results) {
    return results.oauth3.session.refreshToken;
  });
};

DDNS.login = function (options, cli) {
  function log() {
...
DDNS.update = function (options, cli) {
  ...
  function setRecord(Oauth3, oauth3) {
    log('setRecord');
  ...
  function setDevice(Oauth3, oauth3) {
    log('setDevice');
  ...
  return DDNS.refreshToken(options, cli).then(function (results) {
    var Oauth3 = results.Oauth3;
    var oauth3 = results.oauth3;
    ...
    return Oauth3.Domains.all(oauth3).then(function (results) {
      ...
      if (!names.some(function (domain) {
        ...
        return Oauth3.Domains.search(oauth3, {
          ...
          var err = new Error("domain not '" + fullSld + "' registered to account");
      ...
      if (type && -1 === ['A', 'AAAA'].indexOf(type)) {
        return setRecord(Oauth3, oauth3);
      }
      ...
      return p.then(function (ip) {
        options.value = ip;
        return setDevice(Oauth3, oauth3);
      });
    });
  });
};
